...
if [ -z $1 ]; then
  echo "Script cannot run if the Module is not given"
  exit 1
else
  MODULE=$1
fi
...
TEMPLATE="$PARENT_DIR/bicep/modules/public/$MODULE/test/main.test.bicep"
PARAMETERS="$PARENT_DIR/bicep/modules/public/$MODULE/test/parameters.json"
