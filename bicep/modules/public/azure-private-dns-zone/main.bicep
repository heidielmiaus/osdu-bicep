...
module privateDnsZone_virtualNetworkLinks '.bicep/virtual_network_links.bicep' = [for (virtualNetworkLink, index) in virtualNetworkLinks: {
  ...
  params: {
    ...
    resourceName: contains(virtualNetworkLink, 'name') && !empty(virtualNetworkLink.name) ? virtualNetworkLink.name : '${last(split(virtualNetworkLink.virtualNetworkResourceId, '/'))}_link_to_vnet'
